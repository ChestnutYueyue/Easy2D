...
**注意**：引擎已自动处理OpenGL纹理坐标与图片像素坐标的Y轴翻转，您只需按图片视觉布局使用即可。

```
+---+---+---+---+  第1行 (索引0-3): 向下走（正面，人物朝向你）
| 0 | 1 | 2 | 3 |
+---+---+---+---+  第2行 (索引4-7): 向左走（左侧面）
| 4 | 5 | 6 | 7 |
+---+---+---+---+  第3行 (索引8-11): 向右走（右侧面）
| 8 | 9 |10 |11 |
+---+---+---+---+  第4行 (索引12-15): 向上走（背面，人物背对你）
|12 |13 |14 |15 |
+---+---+---+---+
```
...
> **注意**：引擎已自动处理OpenGL纹理坐标的Y轴翻转。您只需按照图片的视觉布局（从上到下）使用帧索引即可。
...
// 方向枚举 - 顺序与精灵图布局匹配
// 精灵图布局：0-3下(正面), 4-7左, 8-11右, 12-15上(背面)
enum class Direction {
    Down,   // 0 -> 帧 0-3 (向下走，人物正面朝向你)
    Left,   // 1 -> 帧 4-7 (向左走)
    Right,  // 2 -> 帧 8-11 (向右走)
    Up      // 3 -> 帧 12-15 (向上走，人物背对你)
};
