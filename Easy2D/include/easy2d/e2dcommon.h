...
// 函数对象
template <typename _Fty> using Function = std::function<_Fty>;

template <class Interface> inline void SafeRelease(Interface *&p) {
  if (p != nullptr) {
    p->Release();
    p = nullptr;
  }
...
// ANSI 字符串转 UTF-8 字符串
ByteString AnsiToUtf8(const ByteString &str);

// UTF-8 字符串转 ANSI 字符串
ByteString Utf8ToAnsi(const ByteString &str);

// ANSI 字符串转宽字符串
WideString AnsiToWide(const ByteString &str);

// 宽字符串转 ANSI 字符串
ByteString WideToAnsi(const WideString &str);
...
struct DrawingStyle {
  ...
  // 绘图样式
  DrawingStyle(Mode mode = Mode::Fill, Color fillColor = Color::White,
               Color strokeColor = Color::Transparent, float strokeWidth = 2.0f,
               LineJoin lineJoin = LineJoin::None);

  // 纯色无描边样式
  DrawingStyle(Color fillColor);
...
public:
  ...
  struct Data {
    ...
    friend std::basic_ostream<Elem> &operator<<(std::basic_ostream<Elem> &out,
      ...
      if (!ok) {
        ...
        if (data.buffer && data.size) {
          out.write(reinterpret_cast<const Elem *>(data.buffer),
...
public:
  ...
  // 名称是否相同
  bool isName(const String &name, size_t hashName = 0) const;
...
public:
  // 加载图片文件
  static Image *load(const String &filePath /* 图片文件路径 */
  );

  // 加载图片资源
  static Image *load(const Resource &res /* 图片资源 */
  );

  // 加载图片资源
  static Image *load(int resNameId,        /* 图片资源名称 */
...
namespace std {
template <> struct hash<easy2d::Resource> {
