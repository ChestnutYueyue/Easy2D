...
public:
  template <typename T, typename... Args> static T *acquire(Args &&...args) {
    return ObjectPoolManager::getInstance().getPool<T>()->acquire(
        std::forward<Args>(args)...);
  }

  template <typename T> static void release(T *obj) {
...
protected:
  ...
  // 开始动作
  void _startWithTarget(Node *target);
...
public:
  explicit MoveBy(float duration, /* 持续时长 */
                  Vector2 vector  /* 移动距离 */
  );
...
public:
  explicit MoveTo(float duration, /* 持续时长 */
                  Point pos       /* 目的坐标 */
  );
...
public:
  explicit JumpBy(float duration,     /* 持续时长 */
...
public:
  explicit JumpTo(float duration,   /* 持续时长 */
...
public:
  explicit ScaleBy(float duration, /* 持续时长 */
                   float scale     /* 相对变化值 */
  );
...
public:
  explicit ScaleTo(float duration, /* 持续时长 */
                   float scale     /* 目标值 */
  );
...
// 透明度相对渐变动作
class OpacityBy : public FiniteTimeAction {
public:
  explicit OpacityBy(float duration, /* 持续时长 */
                     float opacity   /* 相对变化值 */
  );
...
// 透明度渐变动作
class OpacityTo : public OpacityBy {
public:
  explicit OpacityTo(float duration, /* 持续时长 */
                     float opacity   /* 目标值 */
  );
...
// 淡入动作
class FadeIn : public OpacityTo {
public:
  // 创建淡入动作
  explicit FadeIn(float duration /* 持续时长 */
                  )
      : OpacityTo(duration, 1) {}
};

// 淡出动作
class FadeOut : public OpacityTo {
public:
  // 创建淡出动作
  explicit FadeOut(float duration /* 持续时长 */
                   )
...
public:
  explicit RotateBy(float duration, /* 持续时长 */
...
public:
  explicit RotateTo(float duration, /* 持续时长 */
                    float rotation  /* 目标值 */
  );
...
public:
  explicit Delay(float duration /* 延迟时长（秒） */
  );
...
public:
  explicit Loop(Action *action, /* 执行循环的动作 */
                int times = -1  /* 循环次数 */
  );
...
public:
  explicit CallFunc(const Function<void()> &func /* 函数对象 */
  );
...
public:
  ...
  explicit Sequence(const std::vector<Action *> &actions /* 动作列表 */
  );
  ...
  // 在结尾添加多个动作
  void add(const std::vector<Action *> &actions /* 动作列表 */
  );
...
public:
  ...
  explicit Spawn(const std::vector<Action *> &actions /* 动作列表 */
  );
...
public:
  ...
  explicit FrameSequence(const std::vector<KeyFrame *> &frames /* 关键帧数组 */
  );

  explicit FrameSequence(float interval /* 帧间隔（秒） */
  );
  ...
  // 添加关键帧
  void add(KeyFrame *frame /* 关键帧 */
  );

  // 添加关键帧
  void add(Image *image /* 关键帧 */
  );

  // 添加多个关键帧
  void add(const std::vector<KeyFrame *> &frames /* 关键帧列表 */
  );

  // 添加多个关键帧
  void add(const std::vector<Image *> &frames /* 关键帧列表 */
  );
  ...
  // 设置每一帧的时间间隔
  void setInterval(float interval /* 帧间隔（秒） */
  );
...
public:
  ...
  explicit Animation(FrameSequence *seq);
  ...
  // 设置动画
  virtual void setFrameSequence(FrameSequence *seq);
