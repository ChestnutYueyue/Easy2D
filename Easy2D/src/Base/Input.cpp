...
bool Input::__init() {
  ...
  // 初始化接口对象
  HRESULT hr =
      DirectInput8Create(HINST_THISCOMPONENT, DIRECTINPUT_VERSION,
                         IID_IDirectInput8, (void **)&s_pDirectInput, nullptr);

  if (SUCCEEDED(hr)) {
    // 初始化键盘设备
    hr = s_pDirectInput->CreateDevice(GUID_SysKeyboard, &s_KeyboardDevice,
                                      nullptr);

    if (SUCCEEDED(hr)) {
      s_KeyboardDevice->SetCooperativeLevel(
          Window::getHWnd(), DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
      s_KeyboardDevice->SetDataFormat(&c_dfDIKeyboard);
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
      s_MouseDevice->SetCooperativeLevel(Window::getHWnd(),
...
void easy2d::Input::__update() {
  if (s_KeyboardDevice) {
    ...
    if (FAILED(hr)) {
      ...
      s_KeyboardDevice->GetDeviceState(sizeof(s_KeyBuffer),
                                       (void **)&s_KeyBuffer);
    }
  }
  ...
  if (s_MouseDevice) {
    ...
    if (FAILED(hr)) {
      ...
      s_MouseDevice->GetDeviceState(sizeof(s_MouseState),
                                    (void **)&s_MouseState);
    }
  }
...
float Input::getMouseX() { return (float)s_MousePosition.x; }

float Input::getMouseY() { return (float)s_MousePosition.y; }

Point Input::getMousePos() {
  return Point((float)s_MousePosition.x, (float)s_MousePosition.y);
}

float Input::getMouseDeltaX() { return (float)s_MouseState.lX; }

float Input::getMouseDeltaY() { return (float)s_MouseState.lY; }

float Input::getMouseDeltaZ() { return (float)s_MouseState.lZ; }
