...
easy2d::JumpBy::JumpBy(float duration, const Vector2 &vec, float height,
                       int jumps)
    : FiniteTimeAction(duration), _deltaPos(vec), _height(height),
      _jumps(jumps) {}
...
void easy2d::JumpBy::_update() {
  ...
  if (_target) {
    float frac = std::fmod(_delta * _jumps, 1.0f);
