...
Ptr<GLShader> ShaderPreset::Distortion(const DistortionParams& params) {
    ...
    shader->setFloat("u_speed", params.speed);
    shader->setFloat("u_scale", params.scale);
...
Ptr<GLShader> ShaderPreset::Sepia(const SepiaParams& params) {
    auto shader = std::make_shared<GLShader>();
    
    if (!shader->compileFromSource(ShaderSource::StandardVert, ShaderSource::SepiaFrag)) {
        E2D_ERROR("编译复古Shader失败");
        return nullptr;
    }
...
const char* ShaderPreset::GetStandardVertexShader() {
    return ShaderSource::StandardVert;
}

const char* ShaderPreset::GetStandardVertexShaderWithUV() {
    return ShaderSource::StandardVert;
}
