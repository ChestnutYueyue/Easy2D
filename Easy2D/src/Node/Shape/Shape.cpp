...
easy2d::Shape::Shape(ID2D1Geometry *geo) : _geo(geo) {
  if (_geo) {
    _geo->AddRef();
  }
}

easy2d::Shape::~Shape() { SafeRelease(_geo); }
...
easy2d::Shape::Shape(RectType, const easy2d::Rect &rect) {
  ...
  HRESULT hr = Renderer::getID2D1Factory()->CreateRectangleGeometry(
      reinterpret_cast<const D2D1_RECT_F &>(rect), &geo);
  if (FAILED(hr)) {
...
easy2d::Shape::Shape(RoundedRectType, const easy2d::Rect &rect,
                     const Vector2 &radius) {
  ID2D1RoundedRectangleGeometry *geo = nullptr;
  HRESULT hr = Renderer::getID2D1Factory()->CreateRoundedRectangleGeometry(
      D2D1::RoundedRect(reinterpret_cast<const D2D1_RECT_F &>(rect), radius.x,
                        radius.y),
      &geo);
  if (FAILED(hr)) {
...
easy2d::Shape::Shape(EllipseType, const Point &center, const Vector2 &radius) {
  ...
  HRESULT hr = Renderer::getID2D1Factory()->CreateEllipseGeometry(
      D2D1::Ellipse(reinterpret_cast<const D2D1_POINT_2F &>(center), radius.x,
                    radius.y),
      &geo);
  if (FAILED(hr)) {
...
easy2d::Rect easy2d::Shape::getBoundingBox(const Matrix32 *transform) const {
  ...
  if (_geo) {
    ...
    if (transform) {
      D2D1::Matrix3x2F d2dMatrix = transform->toD2DMatrix();
      _geo->GetBounds(&d2dMatrix,
                      reinterpret_cast<D2D1_RECT_F *>(&bounds));
    } else {
      _geo->GetBounds(nullptr, reinterpret_cast<D2D1_RECT_F *>(&bounds));
    }
  }
  return bounds;
}

bool easy2d::Shape::containsPoint(const Point &point,
                                  const Matrix32 *transform) const {
  if (_geo) {
    BOOL ret = 0;
    // no matter it failed or not
    if (transform) {
      D2D1::Matrix3x2F d2dMatrix = transform->toD2DMatrix();
      _geo->FillContainsPoint(reinterpret_cast<const D2D_POINT_2F &>(point),
                              &d2dMatrix,
                              D2D1_DEFAULT_FLATTENING_TOLERANCE, &ret);
    } else {
      _geo->FillContainsPoint(reinterpret_cast<const D2D_POINT_2F &>(point),
                              nullptr, D2D1_DEFAULT_FLATTENING_TOLERANCE, &ret);
    }
    return !!ret;
  }
  return false;
...
bool easy2d::Shape::computePointAtLength(float length, Point &point,
                                         Vector2 &tangent) const {
  if (_geo) {
    HRESULT hr =
        _geo->ComputePointAtLength(length, D2D1::Matrix3x2F::Identity(),
