...
public:
  ...
  bool open(int resNameId, const String &resType);

  bool play(int nLoopCount = 0);
...
easy2d::Music::Media::Media()
    : _opened(false), _playing(false), _loopCount(0), _pResourceData(nullptr),
      _resourceDataSize(0) {
  memset(&_sound, 0, sizeof(_sound));
  memset(&_decoder, 0, sizeof(_decoder));
}

easy2d::Music::Media::~Media() { close(); }
...
bool easy2d::Music::Media::_initSoundFromDecoder() {
  ...
  ma_result result = ma_sound_init_from_data_source(
      s_pEngine, &_decoder, MA_SOUND_FLAG_DECODE, nullptr, &_sound);
...
bool easy2d::Music::Media::open(const easy2d::String &filePath) {
  ...
  ma_result result =
      ma_sound_init_from_file(s_pEngine, actualFilePath.c_str(),
                              MA_SOUND_FLAG_DECODE, nullptr, nullptr, &_sound);
...
bool easy2d::Music::Media::open(int resNameId, const easy2d::String &resType) {
  ...
  if (nullptr ==
      (hResInfo = FindResourceA(HINST_THISCOMPONENT,
                                MAKEINTRESOURCEA(resNameId), resType.c_str())))
  ...
  // 初始化 decoder
  ma_result result = ma_decoder_init_memory(_pResourceData, _resourceDataSize,
                                            nullptr, &_decoder);
...
public:
  ...
  static LRESULT WINAPI MciProc(HWND hWnd, UINT Msg, WPARAM wParam,
                                LPARAM lParam);
...
easy2d::Music::Media::Media()
    : _wnd(NULL), _dev(0L), _playing(false), _repeatTimes(0) {
  _wnd = CreateWindowExA(WS_EX_APPWINDOW, WIN_CLASS_NAME, NULL, WS_POPUPWINDOW,
                         0, 0, 0, 0, NULL, NULL, s_hInstance, NULL);
...
bool easy2d::Music::Media::open(const String &pFileName) {
  ...
  MCIERROR mciError = mciSendCommandA(0, MCI_OPEN, MCI_OPEN_ELEMENT,
...
bool easy2d::Music::Media::open(int resNameId, const String &resType) {
  E2D_ERROR("Music::open failed! Play sound from memory is not supported when "
            "use E2D_WIN7 macro");
  return false;
}
...
bool easy2d::Music::Media::play(int nLoopCount) {
  ...
  // 播放声音
  MCIERROR mciError = mciSendCommandA(_dev, MCI_PLAY, MCI_FROM | MCI_NOTIFY,
...
bool easy2d::Music::Media::isPlaying() const { return _playing; }
...
void easy2d::Music::Media::_sendCommand(int nCommand, DWORD_PTR param1,
                                        DWORD_PTR parma2) {
  // 空设备时忽略这次操作
  if (!_dev) {
    return;
...
LRESULT WINAPI easy2d::Music::Media::MciProc(HWND hWnd, UINT Msg, WPARAM wParam,
                                             LPARAM lParam) {
  easy2d::Music::Media *pMusic = NULL;

  if (Msg == MM_MCINOTIFY && wParam == MCI_NOTIFY_SUCCESSFUL &&
      (pMusic =
           (easy2d::Music::Media *)GetWindowLongPtr(hWnd, GWLP_USERDATA))) {
    if (pMusic->_repeatTimes > 0) {
      pMusic->_repeatTimes--;
    }

    if (pMusic->_repeatTimes) {
      mciSendCommandA(static_cast<MCIDEVICEID>(lParam), MCI_SEEK,
                      MCI_SEEK_TO_START, 0);

      MCI_PLAY_PARMS mciPlay = {0};
      mciPlay.dwCallback = reinterpret_cast<DWORD_PTR>(hWnd);
      mciSendCommandA(static_cast<MCIDEVICEID>(lParam), MCI_PLAY, MCI_NOTIFY,
                      reinterpret_cast<DWORD_PTR>(&mciPlay));
    } else {
...
easy2d::Music::Music() : _media(new Music::Media()) {}

easy2d::Music::Music(const easy2d::String &filePath) : Music() {
  this->open(filePath);
}

easy2d::Music::Music(int resNameId, const String &resType) : Music() {
...
bool easy2d::Music::play(int nLoopCount) { return _media->play(nLoopCount); }

void easy2d::Music::pause() { _media->pause(); }

void easy2d::Music::resume() { _media->resume(); }

void easy2d::Music::stop() { _media->stop(); }

void easy2d::Music::close() { _media->close(); }

bool easy2d::Music::isPlaying() const { return _media->isPlaying(); }
