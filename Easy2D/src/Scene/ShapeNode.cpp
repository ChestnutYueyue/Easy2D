...
void easy2d::ShapeNode::onRender()
{
	...
	// 将显示透明度应用到颜色
	DrawingStyle styleWithOpacity = _style;
	styleWithOpacity.fillColor.a *= _displayOpacity;
	styleWithOpacity.strokeColor.a *= _displayOpacity;
	...
	if (_style.mode == DrawingStyle::Mode::Fill || _style.mode == DrawingStyle::Mode::Solid)
	{
		// 绘制填充
		glRenderer->setOpacity(_displayOpacity);
		glRenderer->drawFilledRect(bounds, _style.fillColor);
	}
}
...
easy2d::Rect easy2d::ShapeNode::getBoundingBox() const
{
	if (_shape)
	{
		Matrix32 transform = getTransform();
