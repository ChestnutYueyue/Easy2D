...
void easy2d::Text::onRender()
{
	if (_layout)
	{
		// 将显示透明度应用到填充颜色的 alpha 通道
		Color fillColor = _style.fillColor;
		fillColor.a *= _displayOpacity;
		
		// 使用GLTextRenderer渲染文本
		GLTextRenderer& textRenderer = GLTextRenderer::getInstance();
		Point pos(0, 0);
		textRenderer.renderText(_layout->getText(), pos, _layout->getStyle(), fillColor);
	}
}
